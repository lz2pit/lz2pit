...
import { ZODIAC_SIGNS, PLANETS, ASPECTS } from "@/lib/astrology-constants";
...
export default function NatalChart({ data }: NatalChartProps) {
  ...
  useEffect(() => {
    ...
    // House lines
    Object.keys(data.houses).forEach((houseKey) => {
      const houseNumber = parseInt(houseKey);
      if (isNaN(houseNumber) || houseNumber > 12) return;
      
      const house = data.houses[houseKey];
      if (house && typeof house.cusp === 'number') {
        ...
        const labelX = center.x + Math.cos(labelAngle) * radius * 0.85;
        const labelY = center.y + Math.sin(labelAngle) * radius * 0.85;
